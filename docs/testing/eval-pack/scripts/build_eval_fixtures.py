...
DEFAULT_SOURCE = REPO_ROOT / "docs/testing/eval-pack/sample-contract-notes.md"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "output/eval-pack/fixtures"
...
def parse_fixtures(source_path: Path) -> list[DraftFixture]:
    ...
    if not fixtures:
        raise ValueError(f"No draft fixtures found in {source_path}")
    return fixtures


def _build_fixture(title: str, lines: list[str]) -> DraftFixture:
    filename = f"redline-eval-{_slugify_title(title)}.docx"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build local DOCX fixtures for the Redline EN eval pack.")
    parser.add_argument("--source", type=Path, default=DEFAULT_SOURCE, help="Path to sample-contract-notes.md")
