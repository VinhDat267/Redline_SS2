  ...
  const stats = [
    {
      ...
      value: compareRun?.source_version?.version_label ?? "Loading...",
      icon: FolderDown
    },
    {
      label: "Target Version",
      value: compareRun?.target_version?.version_label ?? "Loading...",
      icon: ArrowUpToLine
    },
    {
      label: "Changes",
      ...
      <section aria-label="Clause delta" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#F4F5F7' }}>
        ...
        {(error || aiMessage) && (
          <div style={{ flexShrink: 0, padding: '8px 20px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {error && <div style={{ padding: '8px 14px', borderRadius: '8px', background: '#FFF1F0', border: '1px solid #F6465D33', color: '#C03050', fontSize: '12px' }}>{error}</div>}
